...
int demo_kleursystemen() { // -2. Puur voor de demo. Deze functie is geen onderdeel van de opdrachten.
    Mat src, hsv, h, s, v;

    String source_window = "Originele plaatje";         // Mag je weglaten, alleen imshow("naam",src) is voldoende. Dit maakt het mooier.
    String h_window = "Hue";
    String s_window = "Saturation";
    String v_window = "Value";
    ...
    src = imread(binroot+"hartjes.jpg", IMREAD_ANYCOLOR); // plaatje zit in project directory, zie Solution Explorer, is toegevoegd via Shift+Alt+A (Add Existing Item)
    cvtColor(src, hsv, COLOR_BGR2HSV); // plaatje van standaard rgb (bgr) plaatje omzetten naar hsv
    Mat chan[3];
    split(hsv, chan);
    h = chan[0];
    s = chan[1];
    ...
    namedWindow(h_window, WINDOW_AUTOSIZE);
    namedWindow(s_window, WINDOW_AUTOSIZE);
    namedWindow(v_window, WINDOW_AUTOSIZE);
...
int demo_met_camera() { // -1. Puur voor de demo, met histogram. Deze functie is geen onderdeel van de opdrachten.
    String destination_window = "Bronplaatje";
    Mat frame;
    ...
    while (1) {
        ...
        if (key == 'g') { // *.pgm plaatje opslaan; grijswaarden
            ...
            Mat gray_image = Mat::ones(HEIGHT, WIDTH, CV_8U) * 0;   // Maak een nieuw plaatje van hetzelfde type als src, gevuld met 1 * 0 (dus nullen, dus zwart)
...
int main(int argc, const char** argv){
    ...
    do {
        ...
        cout << "-2: Demo kleursystemen" << endl;
        ...
        switch (optie) {
        case -2: ret = demo_kleursystemen(); break;
